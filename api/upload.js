...
export default async function handler(req, res) {
  ...
  // Supprimer une vidéo sur Mux + Supabase
  if (req.method === 'DELETE' && req.query.action === 'delete-video') {
    try {
      ...
      // 1. Supprimer sur Mux
      if (asset_id) {
        await fetch(`https://api.mux.com/video/v1/assets/${asset_id}`, {
          method: 'DELETE',
          headers: { 'Authorization': authHeader }
        });
      }
      ...
      if (video_id && supabase_token) {
        await fetch(`https://emiuzigtlomlrsofxqyf.supabase.co/rest/v1/videos?id=eq.${video_id}`, {
          method: 'DELETE',
          headers: {
            'apikey': 'sb_publishable_U8P52zBkzF6Nqno_mmbcVQ_bKQLNmTP',
