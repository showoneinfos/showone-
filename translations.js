...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // ── Injecter dans la ligne 1 mobile ──
  const mobileL1 = document.getElementById('navMobileLigne1');
  if (mobileL1 && !mobileL1.querySelector('.lang-dropdown-wrap')) {
    // Remplacer le <select> statique par le vrai sélecteur dynamique
    const selectStatic = mobileL1.querySelector('select');
    if (selectStatic) selectStatic.remove();
    const selMobile = createLangSelector('dark');
    mobileL1.appendChild(selMobile);
  }

  // ── Injecter dans side-form (pages connexion/inscription) ──
  ...
  // ── Appliquer les traductions avec la langue sauvegardée ──
  applyTranslations(currentLang);

  // ── Synchroniser TOUS les sélecteurs sur la bonne langue ──
  document.querySelectorAll('.lang-trigger').forEach(trigger => {
    trigger.querySelector('.lang-flag').textContent = LANGS[currentLang].flag;
    trigger.querySelector('.lang-name').textContent = LANGS[currentLang].name;
    trigger.dataset.lang = currentLang;
  });
});
